...
class MCP4786:
    def __init__(self):
        ...
        os.system("echo 1212 | sudo -S chmod 666 /dev/" +
                  str(port).split(' ')[2])
        # 打开串口 (COM3)
        self.__ser = serial.Serial(timeout=1, port="COM3",
...
if __name__ == '__main__':
    robot_right = Robot('192.168.1.102', rotation_y_deg=50.3)
    robot_right.go_home()
    robot_left = Robot('192.168.1.101', rotation_y_deg=-50.3)
    robot_left.go_home()
    #%%
    # robot_right.move_tool_xyz(x= 0.25, y = 0, z = 0)
    # robot_left.move_tool_xyz(x= -0.25, y = 0, z = 0)
    #%%
    mcp = MCP4786()
    time.sleep(2)
    # %%
